...
def adiabatic_wall_BC(A,C,R,Tg,Tyz,Pg,P):
    a = I.a
    b = I.b
    c = I.c
        ...
        for j in range(0,b):# Side Walls
            ...
            if(np.mod(j,2) == 0): # Type 2 Solid node
                ...
                if(i == 0): # Nodes along y axis at x = 0
                    ...
                    C02 = 1/(R[i,j,0,0,0] + R[i+1,j,0,0,1])
                    C22 = 1/(R[i,j,0,2,0] + R[i,j,1,2,1])                    

                    C0_0 = C0_0 -C02 -C22 
                    A[index0,find.find_index(i+1,j,0)] = C02# i+1 term
                    A[index0,find.find_index(i,j,1)] = C22# k+1 term

                    C02 = 1/(R[i,j,c-1,0,0] + R[i+1,j,c-1,0,1])
                    C21 = 1/(R[i,j,c-1,2,1] + R[i,j,c-2,2,0])
                    ...
                    C01 = 1/(R[i,j,0,0,1] + R[i-1,j,0,0,0])
                    C22 = 1/(R[i,j,0,2,0] + R[i,j,1,2,1])   

                    C0_0 = C0_0 -C01 -C22 
                    A[index0,find.find_index(i-1,j,0)] = C01# i-1 term
                    A[index0,find.find_index(i,j,1)] = C22# k+1 term

                    C01 = 1/(R[i,j,c-1,0,1] + R[i-1,j,c-1,0,0])
                    C21 = 1/(R[i,j,c-1,2,1] + R[i,j,c-2,2,0])

                    C0_1 = C0_1 -C01 -C21 
                    A[index1,find.find_index(i-1,j,c-1)] = C01# i-1 term
                    A[index1,find.find_index(i,j,c-2)] = C21# k-1 term
...
####################################################################################################                
            if(np.mod(j,2) == 1): # Type 3 Solid node
                ...
                if(i == 0): 
                    ...
                    C02 = 1/(R[i,j,0,0,0] + R[i+1,j,0,0,1])
                    C22 = 0.5/(R[i,j,0,2,0] + R[i,j,1,2,1])
                
                    C0_0 = C0_0 -C02 - 2*C22
                    A[index0,find.find_index(i+1,j,0)] = C02# Tg i+1,j,k term

                    C02 = 1/(R[i,j,c-1,0,0] + R[i+1,j,c-1,0,1])
                    C21 = 0.5/(R[i,j,c-1,2,1] + R[i,j,c-2,2,0])
...
# Top and bottom wall
        for k in range(0,c):# Top wall
            ...
            if(np.mod(k,2) == 1): # Type 1 Solid node
                ...
                if(i == 0):
                    C02 = 1/(R[i,0,k,0,0] + R[i+1,0,k,0,1] )
                    C12 = 0.5/(R[i,0,k,1,0] + R[i,1,k,1,1])
                    C0_0 = C0_0 - C02 - 2*C12
                    ...
                    C02 = 1/(R[i,b-1,k,0,0] + R[i+1,b-1,k,0,1] )
                    C11 = 0.5/(R[i,b-1,k,1,1] + R[i,b-2,k,1,0])
                    C0_1 = C0_1 - C02 - 2*C11
                    ...
                    C01 = 1/(R[i,0,k,0,1] + R[i-1,0,k,0,0] )
                    C12 = 0.5/(R[i,0,k,1,0] + R[i,1,k,1,1])
                    C0_0 = C0_0 - C01 - 2*C12
                    ...
                    C01 = 1/(R[i,b-1,k,0,1] + R[i-1,b-1,k,0,0] )
                    C11 = 0.5/(R[i,b-1,k,1,1] + R[i,b-2,k,1,0])
                    C0_1 = C0_1 - C01 - 2*C11
                ...
                if(k == 0):
                    C22 = 1/(R[i,0,k,2,0] + R[i,0,k+1,2,1] )
                    C0_0 = C0_0 - C22
                    A[index0,find.find_index(i,0,k+1)] = C22
                    
                    C22 = 1/(R[i,b-1,k,2,0] + R[i,b-1,k+1,2,1] )
                    C0_1 = C0_1 - C22
                    A[index1,find.find_index(i,b-1,k+1)] = C22
                    
                    
                elif(k == c-1):
                    C21 = 1/(R[i,0,k,2,1] + R[i,0,k-1,2,0] )
                    C0_0 = C0_0 - C21
                    A[index0,find.find_index(i,0,k-1)] = C21
                    
                    C21 = 1/(R[i,b-1,k,2,1] + R[i,b-1,k-1,2,0] )
                    C0_1 = C0_1 - C21
