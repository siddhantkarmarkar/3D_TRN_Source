    ...
    for index in range(0,np.size(C)):
        ...
        if(j != 0 and j!= b-1 and k != 0 and k != c-1): # except boundary walls
            ...
            if(np.mod(j,4) == 1 and np.mod(k,2) == 1): # Hot channel
                ...
                # print(index)
                C11 = 1/(R[i,j-1,k,1,0] + R[i,j,k,1,1])
                C12 = 1/(R[i,j+1,k,1,1] + R[i,j,k,1,0])
                C21 = 1/(R[i,j,k,2,1] + R[i,j,k-1,2,0])
                C22 = 1/(R[i,j,k,2,0] + R[i,j,k+1,2,1])
                C01 = -0.5*(C11 +C12+ C21 +C22) + I.m_channel*find.Cp(Tg[i,j,k],Pg[i,j,k],i,j,k)   
                C02 = -0.5*(C11 +C12+ C21 +C22) - I.m_channel*find.Cp(Tg[i,j,k],Pg[i,j,k],i,j,k)                                                        
...
#Wall nodes                    
            ...
            if(np.mod(j,2) == 0 and np.mod(k,2) == 1): # Type 1 Solid node
                ...
                C11 = 0.5/(R[i,j,k,1,1] + R[i,j-1,k,1,0])
                C12 = 0.5/(R[i,j,k,1,0] + R[i,j+1,k,1,1])
                C21 = 1/(R[i,j,k,2,1] + R[i,j,k-1,2,0] )
                C22 = 1/(R[i,j,k,2,0] + R[i,j,k+1,2,1] )
 
                if(i == 0):# Hot in BC
                    C02 = 1/(R[i,j,k,0,0] + R[i+1,j,k,0,1] )
            ...
            if(np.mod(j,2) == 0 and np.mod(k,2) == 0 ): # Type 2 Solid node
                ...
                C11 = 1/(R[i,j,k,1,1] + R[i,j-1,k,1,0])
                C12 = 1/(R[i,j,k,1,0] + R[i,j+1,k,1,1])
                C21 = 1/(R[i,j,k,2,1] + R[i,j,k-1,2,0])
                C22 = 1/(R[i,j,k,2,0] + R[i,j,k+1,2,1])
                if(i == 0):
                        C02 = 1/(R[i,j,k,0,0] + R[i+1,j,k,0,1])
            ...
            if(np.mod(j,2) == 1 and np.mod(k,2) == 0 ): # Type 3 Solid node
                ...
                C11 = 1/(R[i,j,k,1,1] + R[i,j-1,k,1,0])
                C12 = 1/(R[i,j,k,1,0] + R[i,j+1,k,1,1])
                C21 = 0.5/(R[i,j,k,2,1] + R[i,j,k-1,2,0])
                C22 = 0.5/(R[i,j,k,2,0] + R[i,j,k+1,2,1])
    ...
    A,C = BC.adiabatic_wall_BC(A,C,R,Tg,Tyz,Pg,P)                
    return(A,C)
